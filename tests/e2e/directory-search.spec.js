...
test("searching G finds GBrain and GStack across all families", async ({ page }) => {
  ...
  await expect(page.locator("#all-directory-result-count")).toContainText("151 entries · Scores hidden across collections");
...
test("inference services combine filters and expose the dedicated service score", async ({ page }) => {
  ...
  await expect(page.locator("#inference-result-count")).toContainText("40 services · Inference-service score");
