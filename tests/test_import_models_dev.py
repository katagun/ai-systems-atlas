...
class ModelsDevImportTests(unittest.TestCase):
    def test_json_fetch_rejects_hosts_that_only_contain_an_allowed_hostname(
        self,
    ) -> None:
        malicious_urls = (
    ...
    def test_archive_fetch_rejects_hosts_that_only_contain_the_allowed_hostname(
        self,
    ) -> None:
        malicious_url = (
    ...
    def test_dispositioned_source_ids_leave_the_queue_but_count_as_eligible(
        self,
    ) -> None:
        catalog = {
            "acme/chat": model_record("acme/chat"),
    ...
    def test_source_snapshot_keeps_every_model_regardless_of_output_modality(
        ...
        self.assertEqual(
            ["acme/chat", "acme/image"], [item["source_id"] for item in records]
        )
        self.assertEqual(
            ["image"], records[1]["source_metadata"]["modalities"]["output"]
        )
    ...
    def test_model_ids_are_stable_and_collection_scoped(self) -> None:
        self.assertEqual(
            "model-google-gemma-4-e2b-it", stable_model_id("google/gemma-4-E2B-it")
        )
