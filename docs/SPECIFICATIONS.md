...
The catalog covers tool and context integration, agent interaction, user interfaces, editor clients, web-page tools, identity and discovery, agent transactions, capability and plugin packaging, plus a bounded set of repository-instruction conventions. OASF is classified as a metadata schema rather than a wire protocol. WebMCP is an evolving Community Group Report, not labeled a W3C Standard. AP2, UCP, and Commerce ACP remain separate because authorization evidence, cross-platform commerce, and agent-to-seller checkout answer different integration questions. The two ACP records use distinct display names: ACP for Agent Client Protocol and Commerce ACP for Agentic Commerce Protocol. Agent Plugins and Claude Code Plugins are both plugin package formats and both remain: Agent Plugins is a portable contract whose version 1 component types are only skills and MCP servers, leaving commands, agents, and hooks to namespaced client extensions, while Claude Code Plugins defines those components itself and adds marketplace distribution. Claude Code does not appear among the portable format's documented compatible clients, so the two records answer different integration questions rather than describing one format twice.
