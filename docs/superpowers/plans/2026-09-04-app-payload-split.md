...
- **Why the complement rule.** Detail is the published record minus the boot fields. A curated drop-list was considered and rejected: an earlier pass proposed dropping nine never-rendered fields from `projects.json`, and `BACKLOG.md` item 36 reaches the opposite conclusion about `current_repo_note`, which carries product-boundary prose `docs/CURATION.md` requires reviewers to write and which the backlog wants rendered. A complement rule cannot lose a field.
