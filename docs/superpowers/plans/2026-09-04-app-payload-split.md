...
### Task 1: Authorize the new artifact class (ADR 026)
...
**Files:**
- Create: `docs/adr/026-app-payloads-are-a-projection-of-the-published-endpoints.md`
...
Create `docs/adr/026-app-payloads-are-a-projection-of-the-published-endpoints.md`. Match the house shape of `docs/adr/013-distinct-collections-share-one-directory-surface.md` — read it first for tone and section headings. It must state:
...
```bash
git add docs/adr/026-app-payloads-are-a-projection-of-the-published-endpoints.md AGENTS.md tests/test_documentation.py
git commit -m "Separate the published endpoints from the app's payload (ADR 026)"
```
...
async function bootstrap() {
  ...
  // a projection of them shaped for a first render. See ADR 026.
...
**Interfaces:**
- Consumes: ADR 026 (Task 1), the builder (Task 2), the loading model (Task 5).
