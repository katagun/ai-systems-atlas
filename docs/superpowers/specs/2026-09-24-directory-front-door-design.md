...
- On a 390 px phone the primary navigation is 12 px wider than its frame and clips "Blog"; `docs/WEB.md` verification step 24 says every tab fits.
...
- **AI labs.** A cloud session is designing an AI labs tracking feature whose design is not yet in the repository. It fits in one of two ways:
  - As its own collection, with a tile, a tab, list rows, a record panel, and its own ADR under ADR 013.
  - As the maker behind records the catalog already has: model developer, inference-service operator, runtime maintainer, robot manufacturer, and system vendor. That gives a Maker facet in the filter rail, a Labs group in search suggestions, and "More from this lab" as related records in the panel, with no score and no cross-collection Compare.
  Phase 3's spec settles which, once the labs design lands. A lab registry could also settle the `BACKLOG.md` item on reconciling inference-service `operator` values.
...
- **ADR 040, "Search orders by match, never by score"**, lands with Phase 1. It amends ADR 013's All bullet ("alphabetical discovery") and ADR 032's "The scope is alphabetical only": both hold while browsing, and a query orders by match. It amends ADR 037's "They are listed alphabetically" the same way, named in the same sentence as ADR 013 and ADR 032. The Robots session, which owns ADR 037, agreed on 2026-09-24. ADR 037's other rules stay untouched: no score, comparison, badge, or Finder goal. The number is claimed; the badge session holds 039.
...
- ADR 040 is claimed here; 039 is theirs.

The Robots session builds ADR 037 on `claude/robots-collection-plumbing`, and Phase 0 starts after its PR merges. The AI labs cloud session can't send messages back to other sessions; it has been asked to record its integration choices in its spec or PR. Two Codex worktrees (`codex/curate-agent-systems-batch-40-41`, `codex/refresh-agent-instructions`) change catalog data and agent instructions, so they only meet this work in generated files.
