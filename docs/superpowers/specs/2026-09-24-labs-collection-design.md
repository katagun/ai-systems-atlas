...
The landing-page redesign session asked how labs plug into its front door, which has collection tiles, results scope tabs, a filter rail, and a record side panel. Its spec, [`2026-09-24-directory-front-door-design.md`](2026-09-24-directory-front-door-design.md), first offered two ways in under "Collections in flight"; the version merged in #299 maps Labs the way these answers do.
...
- **(b) Labs are the maker behind other records, and that join is already built.**
  ...
  - **Robots.** The Robots collection landed in #300 with no records yet, and its `manufacturer` field is not joined. Adding it is one field in `scripts/lab_relations.py` and one in `labRelations`, once there are robots to join.
