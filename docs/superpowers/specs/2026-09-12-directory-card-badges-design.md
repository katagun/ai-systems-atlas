...
Each badge declares a scope: a collection, and for systems a family. A name is shared across scopes only when it tests the same field and value with the same meaning. `Local-first` and `Self-hostable` are shared across the three system families. `Anthropic-compatible API` is shared by inference services and local runtimes because both test `api_styles: anthropic_compatible`. A badge that only resembles another, such as a system's local-first behavior and a model's downloadable weights, gets its own name.
...
| Inference services (59) | Bring your own weights | `model_sources` ∋ `customer_supplied` | 36% |
| | Dedicated endpoints | `delivery_modes` ∋ `dedicated_endpoint` | 37% |
| | Anthropic-compatible API | `api_styles` ∋ `anthropic_compatible` | 31% |
...
| | Developer API | `distribution_modes` ∋ `developer_api` | 68% |
| | Hosted by third parties | `distribution_modes` ∋ `third_party_hosting` | 42% |
...
Reviewed-model cards show their badges, then the modality route and family as plain text that is visibly not a badge, with a `title` stating that the values come from models.dev.

### 6. Every badge explains itself without becoming a control

A badge is a non-focusable `<span>` holding its visible name, a `title` carrying its one-line plain-language definition for pointer readers, and visually hidden text carrying the same definition for screen readers. It adds no tab stop. The Taxonomy view gains a "Card badges" section listing each badge, its definition, and the scopes it applies to, so keyboard and touch readers can find the meaning of any badge.
