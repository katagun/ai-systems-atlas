...
- `headquarters`: the country where the organization says it is headquartered, from its own pages. When a holding company is incorporated elsewhere, record where the organization says it is headquartered and put the incorporation in `organization_note`. When the organization's own pages list offices but name no headquarters, record `none_listed`, shown as "No headquarters listed", and name its governing legal entities in `organization_note`; never infer a country from a registered office, the largest office, or where the founders are. Add a country to the taxonomy's `countries` group only in the change whose record needs it.
...
Fifteen labs cover 237 of the 303 reviewed releases. [`COVERAGE.md`](COVERAGE.md#labs) records which organizations the first batch took and why ByteDance is held, and [`BACKLOG.md`](../BACKLOG.md) carries the second batch and the direct re-read of the first batch's pages.
