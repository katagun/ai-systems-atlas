...
- Choosing a family, or Systems for every family, clears any role or Finder-role constraint; “Clear filters” restores the all-family active-system default.
...
- Clicking anywhere on a card opens its record; the card's Compare control, emblem row, and hover-titled facts stay separate targets above it. The details control's accessible name names the record ("View details for Aider").
...
2. switch memory, agent, assistant, and all-family views; reload with a family chip, a query, and a filter selected and confirm all three return;
...
24. confirm the primary navigation is plain text with an underline under the active view, that every navigation item, Blog included, is fully visible at 390px, 360px, and 320px, and sits on one row at 390px, and that the footer notices and data date align to the content column at desktop and phone widths.
...
26. switch views and confirm the URL gains and drops `view=`, reload a `view=` URL, and confirm an unknown value falls back to the Directory with the parameter removed; confirm a Models or Labs URL restores its query and filters.
