...
- The directory boots from five payloads under `web/` — `app/systems.json`, `app/inference.json`, `app/runtimes.json`, `app/specifications.json`, and `taxonomy.json` — rather than from the published endpoints; see [ADR 025](adr/025-app-payloads-are-a-projection-of-the-published-endpoints.md). A search box loads its collection's search index from `app/search/` on focus. A record loads its own file from `app/detail/<kind>/<id>.json` when its dialog opens, when it enters a comparison, or when a Finder goal chooses it. `license-evidence.json` loads on the first record opened, and `logos.json` loads after first paint. Every one of these degrades to something correct if it never arrives. Because a record's detail is merged into its boot record in place, a dialog opened for the first time paints twice: once from the boot record, once when detail lands.
