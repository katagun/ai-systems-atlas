...
2. For each page you intend to cite, run the two-fetch rule: fetch the page twice and compare the two content hashes (the dedicated stability helper has not landed yet; the remaining plumbing is tracked in the robots item in [`../BACKLOG.md`](../BACKLOG.md)). Paste both hashes into the pull request description either way. When they differ, look for a stable first-party alternative first; if none exists, cite the page with `"unpinnable": true`, which keeps the link checker on it and leaves it out of drift monitoring. An unpinnable page never holds a robot.
