...
2. For each page you intend to cite, run the two-fetch rule: `uv run python scripts/check_page_stability.py <url>`, the script that arrives with the collection plumbing. Paste both hashes into the pull request description either way. When they differ, look for a stable first-party alternative first; if none exists, cite the page with `"unpinnable": true`, which keeps the link checker on it and leaves it out of drift monitoring. An unpinnable page never holds a robot.
