...
`uv run python scripts/build_web_payload.py` writes 288 files under `web/app/`: five boot payloads (`systems.json`, `inference.json`, `runtimes.json`, `specifications.json`, `models.json`), five search indexes under `app/search/`, and 278 per-record detail files under `app/detail/<kind>/<id>.json` — the projection `web/app.js` actually loads at boot, on search focus, and on record or comparison open; see [ADR 026](adr/026-app-payloads-are-a-projection-of-the-published-endpoints.md). `--check` rebuilds the tree in memory and fails when the committed files differ, and `verify.yml` runs it on every pull request. `scripts/update_directory.py` regenerates payloads right after `sync_web_data()`, since payloads project the files that call writes and cannot be built before it. `.github/workflows/update-directory.yml` then runs the builder again as a backstop for a refresh that returned early on an earlier error, followed by `build_share_pages` and `build_asset_version` as separate steps — so the builder runs twice per weekly refresh, which costs duplicated work but nothing else, since it is deterministic and idempotent. The two orderings that matter are enforced in two different places, `sync_web_data` before `build_web_payload` in the Python `main()` and `build_web_payload` before `build_share_pages` before `build_asset_version` in the workflow's step order, and dropping the builder from either one ships stale `stars` on every subsequent run, because `stars` is a card field and keeping it current is the refresh's whole job.
