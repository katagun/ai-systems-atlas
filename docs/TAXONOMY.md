...
Specifications, inference services, local runtimes, models, agent packs, labs, and robots are separate collections, not additional system families. Specifications, agent packs, labs, and robots remain unscored. Inference services, local runtimes, and Atlas-reviewed provider-independent model releases each use their own taxonomy and dedicated score profile, none comparable with a system-family profile or with one another. Systems, the complete attributed models.dev source catalog, services, and runtimes share the mixed Directory only as a presentation union; imported model rows remain unscored, Models remains a sibling specialist view, and Specifications remains a separate artifact view. See [`SPECIFICATIONS.md`](SPECIFICATIONS.md), [`INFERENCE_SERVICES.md`](INFERENCE_SERVICES.md), [`LOCAL_RUNTIMES.md`](LOCAL_RUNTIMES.md), [`MODELS.md`](MODELS.md), [`PACKS.md`](PACKS.md), [`LABS.md`](LABS.md), [ADR 013](adr/013-distinct-collections-share-one-directory-surface.md), [ADR 025](adr/025-model-releases-are-independent-curated-records.md), [ADR 027](adr/027-complete-models-dev-source-catalog-is-published.md), [ADR 032](adr/032-agent-packs-are-unscored-records-of-what-a-host-installs.md), [ADR 037](adr/037-robots-are-unscored-records-of-what-a-vendor-documents.md), and [ADR 041](adr/041-labs-are-unscored-records-of-who-develops-the-catalogs-models.md). A lab's type and headquarters are classification, never rank: `lab_types` says whether AI models are the organization's principal business, and `countries` records where it says it is headquartered, or `none_listed` when its own pages name no headquarters.

A robot is classified by five groups of its own, and never receives a system family or a primary role: `robot_form_factors` (`humanoid`, `quadruped`, `arm`, `mobile_manipulator`, `other`), `robot_ai_bases` (`vendor_named_model` when the maker's documentation names a learned model or policy and says what it controls, `open_model_interface` when the maker documents a supported way to run the reader's own), `robot_availability` (how the vendor says a buyer gets one), `robot_model_kinds` (what kind of model the maker names), and `robot_terms_kinds` (the terms the maker publishes, which are never classified as software licences). See [`ROBOTS.md`](ROBOTS.md).

Installing into a host agent is a deployment mode, `host_pack`, not a collection: a scored system that ships as a skills bundle, plugin, or vault keeps its family, role, and score, and the Agent packs scope lists it inline among the unscored packs. See [ADR 034](adr/034-installing-into-a-host-is-a-deployment-mode-not-a-collection.md) and [ADR 035](adr/035-host-installed-systems-are-listed-inline-in-the-packs-scope.md).
