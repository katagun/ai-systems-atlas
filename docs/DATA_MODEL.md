...
- **First-party anchor:** `first_party_domains`, a non-empty list of bare lowercase hosts or `github.com/<org>` prefixes. A shared host such as `github.io`, `huggingface.co`, or `x.com` is refused, and `github.com` is accepted only as an org-scoped prefix. The record `url`, every evidence `url` and `immutable_url`, and every terms-evidence `url` must fall under an entry, host or subdomain.
