...
- `scripts/check_evidence_links.py` monitors a robot's terms evidence and its named-model page for drift, because the named-model page is the collection's central fact. Drift opens an incident that waits for a human and never hides the record.
