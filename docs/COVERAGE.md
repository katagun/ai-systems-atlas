...
2. **Managed agent platforms:** Microsoft Foundry Agent Service, Copilot Studio, Amazon Bedrock AgentCore, Gemini Enterprise Agent Platform, Salesforce Agentforce, IBM watsonx Orchestrate, and the Meta Business Agent Platform. All seven share one shape: the vendor hosts the runtime while the customer configures an agent with tools, grounding, and handoff. The batch question is which role that shape takes, since `agent_framework_sdk` describes a library the customer runs and these are operated services. Settle it once for the class rather than per vendor.
