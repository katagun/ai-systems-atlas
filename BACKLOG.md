...
- [ ] Confirm the two registered routines complete their first unattended run. The weekly refresh closed this bar already: its first live Monday run under launchd opened [#158](https://github.com/katagun/ai-systems-atlas/pull/158) on 2026-09-14, correctly left in draft because its evidence-link and terms-drift check failed — exactly what [#149](https://github.com/katagun/ai-systems-atlas/pull/149) set as the closing condition. `#158` still needs the user's own review and merge before its metadata refresh (and any candidate or license-review changes) land; its candidate count is unchanged at 148. Of the two routines [#147](https://github.com/katagun/ai-systems-atlas/pull/147) registered, `hn-signals` fired its first scheduled run at 08:16 on 2026-09-14 and was still running over ninety minutes later with no `finish` commit yet, so its outcome is unconfirmed; `candidate-triage` has not reached its first Tuesday window yet (next: 2026-09-15 09:01).
