...
- [ ] Resolve terms drift on two pages the evidence-link checker still flags after two full `--max-age-hours 0` re-crawls run minutes apart on 2026-09-13: DeepInfra's terms page and Baseten's terms and conditions page, each shared between a `terms` entry and a trust-record property dated the same day during the inference-service batches, so a same-day date cannot itself clear it. [#146](https://github.com/katagun/ai-systems-atlas/pull/146) reported DeepInfra's drift already cleared on its own crawl the same day; that could not be reproduced here twice, so read both live pages, decide whether anything material changed, and advance every dated reference sharing each URL together.
...
- [ ] Schedule `scripts/run_directory_refresh.py --publish` weekly with launchd, as the sweep already is. The 2026-09-13 run ([#144](https://github.com/katagun/ai-systems-atlas/pull/144)) was started by hand after metadata had gone stale since 2026-08-24; it surfaced three terms-drift pages as a failed check, which is the intended path for drift review now that none is outstanding. The job must tolerate the runner's refusal on a dirty tree or a `HEAD` that is not fresh `origin/main`, and leave the pull request as a draft when any check fails.
