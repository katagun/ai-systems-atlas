...
- [ ] Hash only the terms content of monitored pages, and migrate their baselines safely. The evidence-link checker now keeps one shared cache and reports missing baselines, but it still hashes a whole page's visible text, so site furniture raises drift with no terms change: DeepInfra's rotating featured-models menu, a Baseten variant, three Cohere pages, and the Claude Agent SDK overview, whose `#license-and-terms` anchor does not narrow the hash. Hash the anchored section when a URL names one and the main terms content otherwise. Changing normalization changes all 134 page baselines at once, so adopt a new hash silently only where the old normalization of the current page still matches the stored baseline, and report every other page as drift.
- [ ] Confirm `candidate-triage` completes its first unattended run; it has not reached its first Tuesday window yet (next: 2026-09-15 09:01). The other two scheduled jobs already closed this bar: the weekly refresh's first live Monday run under launchd opened [#158](https://github.com/katagun/ai-systems-atlas/pull/158) on 2026-09-14, correctly left in draft because its evidence-link and terms-drift check failed, exactly what [#149](https://github.com/katagun/ai-systems-atlas/pull/149) set as the closing condition; and `hn-signals`' first scheduled run completed the same morning and opened [#165](https://github.com/katagun/ai-systems-atlas/pull/165). Both `#158` and `#165` still need the user's own review and merge.
