...
- [ ] Resolve terms drift on three pages the evidence-link checker keeps flagging across three separate `--max-age-hours 0` re-crawls on 2026-09-13, the last two against a warm same-machine cache: DeepInfra's terms page and Baseten's terms and conditions page, each shared between a `terms` entry and a trust-record property dated the same day during the inference-service batches so a same-day date cannot itself clear it; and Claude Agent SDK's license-and-terms anchor, whose evidence has not been reviewed since 2026-08-25. [#146](https://github.com/katagun/ai-systems-atlas/pull/146) reported DeepInfra's drift already cleared on its own crawl the same day; that has not reproduced here in three attempts, so read all three live pages, decide whether anything material changed, and advance every dated reference sharing each URL together.
