...
- [ ] Triage the 127 candidates that carry no `triage` block. The `candidate-triage` routine's first scheduled run started 2026-09-15 at 09:01 and was still running at 11:45; if it commits through `finish`, review its proposals, otherwise run `scripts/run_candidate_triage.py` by hand. Done when every queued candidate has a `review_ready`, `out_of_scope`, or held proposal to start review from.
...
- [ ] Confirm `candidate-triage` completes one unattended run through `finish`. Its first scheduled run started 2026-09-15 at 09:01 and was still running at 11:45, as was `hn-signals`' second daily run, so neither day's outcome is confirmed yet. The weekly refresh and `hn-signals` each closed this bar once already: the refresh's first launchd run opened [#158](https://github.com/katagun/ai-systems-atlas/pull/158), correctly in draft because the Claude Agent SDK page had drifted (resolved by [#170](https://github.com/katagun/ai-systems-atlas/pull/170)), and the signals routine opened [#165](https://github.com/katagun/ai-systems-atlas/pull/165); both are merged. Watch for a run that stays `running` for hours with no commit, which is what a stuck guard would look like from outside.
