...
- [ ] Add the `ATLAS_AUTOMATION_TOKEN` repository secret so the weekly refresh pull request runs `verify`. The repository has no secrets as of 2026-09-12, so every refresh pull request is opened with `GITHUB_TOKEN` and its required check never starts; `docs/OPERATIONS.md` names the token and its two scopes, and the setting has no supported automation. The 2026-09-07 refresh failed on the twenty-four evidence links #121 has since repaired, so issue #107 stays open only until the next scheduled run succeeds, and the draft it opened (#106) should be closed unmerged when one does.
- [ ] Resolve terms drift on DeepInfra's terms page (`https://deepinfra.com/terms`). The evidence-link checker flagged it as changed on 2026-09-12, the same day its terms entry and its trust-record `vulnerability_disclosure` property were both dated during the managed-hosts trust batch; a same-day date cannot itself clear the drift under the checker's newer-than-baseline rule. Read the live page, decide whether anything material changed, and advance both dated references together.
...
- [x] Review trust records for the twelve cloud model platforms, the fourth and final batch under [ADR 029](docs/adr/029-trust-records-are-unscored-and-never-first-hand.md). All fifty-nine inference services now carry a trust record. Two carry findings: Amazon Bedrock (arXiv 2502.07776v2, no caching detected on Claude 3 Haiku in the paper's 2024 audit, which predates and is superseded by Bedrock's documented per-cache-invocation feature) and Vertex AI (the same paper, no caching detected on Gemini 1.5 Flash and text-embedding-004, closed against the current project-isolated, in-memory, 24-hour cache the data-governance page now documents). Bedrock, OCI Generative AI, and Databricks each document that request content is not shared with the third-party model providers whose models they host; Alibaba Cloud Model Studio and Microsoft Foundry Models state their caches are isolated per account or subscription. A cloud-wide compliance page counted only when it named the service: Cloudflare, Google, and Alibaba name theirs; IBM, Oracle, Baidu, BytePlus, Tencent, and Databricks's own SOC page do not, leaving those undocumented despite the parent cloud holding certificates. Two citations from the delegated research were downgraded after the integrator could not independently read the cited page: an Oracle FedRAMP/IL5 blog post that returned a server error on re-fetch, and a Baidu security-response-centre page that rendered empty under both a plain fetch and a browser session. Scores are unchanged throughout.
