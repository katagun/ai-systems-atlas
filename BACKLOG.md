...
- [ ] Resolve terms drift the evidence-link checker keeps raising on two pages, each shared between a `terms` entry and a trust-record property dated the same day during the inference-service batches, so a same-day date cannot itself clear it: DeepInfra's terms page and Baseten's terms and conditions page. Read each live page, decide whether anything material changed, and advance every dated reference sharing that URL together.
