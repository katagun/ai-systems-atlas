...
- [ ] Hash only the terms content of monitored pages, and migrate their baselines safely. The evidence-link checker now keeps one shared cache and reports missing baselines, but it still hashes a whole page's visible text, so site furniture raises drift with no terms change: DeepInfra's rotating featured-models menu, a Baseten variant, three Cohere pages, and, as [#170](https://github.com/katagun/ai-systems-atlas/pull/170) found, the Claude Agent SDK overview, whose `#license-and-terms` anchor does not narrow the hash. Hash the named fragment when a URL has one and the terms body otherwise, never site navigation. Changing normalization changes all 134 page baselines at once, so adopt a new hash silently only where the old normalization of the current page still matches the stored baseline, and report every other page as drift.
