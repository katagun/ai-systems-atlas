...
- [ ] Triage, then work, the candidate queue in small evidence-backed batches. After [#144](https://github.com/katagun/ai-systems-atlas/pull/144) and [#145](https://github.com/katagun/ai-systems-atlas/pull/145) it holds 150: 22 held (thirteen robotics plus the nine #141 holds) and 128 without a triage block — 102 discovered by the 2026-09-13 refresh, five queued from attention-source signals (Graphify C#, gPTY, mobile-use, ARTEMIS, iLands), and the 21 already actionable. Run `scripts/run_candidate_triage.py` over the untriaged set first so review starts from `review_ready` proposals rather than raw discovery.
