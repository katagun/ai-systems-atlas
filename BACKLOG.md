...
- [ ] Review trust records for the remaining twenty-seven inference services: the fifteen managed inference hosts first, then the twelve cloud model platforms, where a platform-wide attestation counts only when it names the service. The direct APIs took one working session for twenty with five parallel research passes; budget the same.
