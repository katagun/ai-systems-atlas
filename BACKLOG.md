...
- [ ] Review mini-swe-agent, then publish SWE-agent as superseded by it. SWE-agent passes the research-artifact test that excluded STORM and Agent S — its loop is in a tested library with continuous integration — but its readme states that mini-swe-agent has superseded it and recommends the successor going forward. ADR 016 records a declared successor only once that successor is published, so the sequence is fixed: the successor first.
