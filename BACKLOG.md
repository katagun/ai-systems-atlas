...
- [ ] Triage the 127 candidates that carry no `triage` block. If the `candidate-triage` run that started 2026-09-15 commits through `finish`, review its proposals; otherwise run `scripts/run_candidate_triage.py` by hand. Done when every queued candidate has a `review_ready`, `out_of_scope`, or held proposal to start review from.
