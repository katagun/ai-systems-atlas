...
- [ ] Narrow noisy terms pages with evidence, not heuristics. The evidence-link checker now stores the text behind every terms hash and prints a diff with each drift report, and anchored URLs hash only their section, but whole pages still hash all their visible text. A generic main-content rule does not work: the 2026-09-15 sample found empty `<main>` elements on the three Cohere pages and OpenRouter, no main landmark on DeepInfra or 34 other pages, and identical hashes on repeat fetches, so drift is real page change rather than per-request noise. As each flagged page's first stored diff arrives, add a `TERMS_SECTION_IDS` entry only where that diff shows the churn sits outside the terms, starting with the 16 pages that already differed from their baselines on 2026-09-15.
...
- [ ] Review the 40 triage proposals the `candidate-triage` run committed to `triage/pending` at 12:03 on 2026-09-15 (20 `review_ready`, 15 `out_of_scope`, 5 held, each with a `finding` and pinned evidence) and land them through a pull request; check each finding against its cited page, not its verdict. That leaves 87 of 148 candidates with no block for the next Tuesday run or a hand run of `scripts/run_candidate_triage.py`. Done when every queued candidate has a proposal to start review from.
...
- [ ] Curate the fifth models.dev queue batch: the newer GPT lines readers migrate toward (`openai/gpt-5-pro`, `openai/gpt-5.1`, `openai/gpt-5.2`, `openai/gpt-5.4`, `openai/gpt-5.6-sol`, `openai/gpt-5.6-terra`, `openai/gpt-5.6-luna`). The 5.6 heads are the named replacements in the batch-four records. Promote or exclude every reviewed candidate in the same change with first-party evidence and the `model_access` score under `docs/MODELS.md`. Codex variants and 5.4 scale variants stay queued for later batches with their own boundary questions.
