...
- [ ] Resolve terms drift on three pages the evidence-link checker keeps flagging across four separate `--max-age-hours 0` re-crawls on 2026-09-13, the last three against a warm same-machine cache: DeepInfra's terms page and Baseten's terms and conditions page, each shared between a `terms` entry and a trust-record property dated the same day during the inference-service batches so a same-day date cannot itself clear it; and Claude Agent SDK's license-and-terms anchor, whose evidence has not been reviewed since 2026-08-25. [#146](https://github.com/katagun/ai-systems-atlas/pull/146) reported DeepInfra's drift already cleared on its own crawl the same day; that has not reproduced here in four attempts, so read all three live pages, decide whether anything material changed, and advance every dated reference sharing each URL together.
- [ ] Make the attention-source and candidate-triage routines actually schedulable, then register them. Neither was ever registered: `~/.claude/scheduled-tasks/{hn-signals,candidate-triage}/SKILL.md` exist only for `prepare`'s prompt-drift check, and every triage run so far was started by hand. As written, [`docs/routines/hn-signals.md`](docs/routines/hn-signals.md) names no working directory and runs `prepare` without `--from-ref`, so an unattended run would build from `origin/main`, where the queue is empty by design. Give each prompt a reviewed way to locate its checkout (and `--from-ref local/hn-signals` for signals) without weakening the exact-match drift guard, then register `hn-signals` daily after the 07:23 sweep and `candidate-triage` on Tuesday mornings. Acceptance: both appear in the desktop app's routines list and one scheduled run of each commits through `finish`.
...
- [ ] Confirm the launchd-scheduled weekly refresh actually publishes once, unattended. [#149](https://github.com/katagun/ai-systems-atlas/pull/149) installed the dedicated worktree, the dirty-tree and stale-`HEAD` guards, and the Monday 07:17 plist, and verified one push-free dry run under launchd; the loop is not closed until a live `--publish` run completes on schedule and either opens a mergeable pull request or leaves one correctly in draft.
